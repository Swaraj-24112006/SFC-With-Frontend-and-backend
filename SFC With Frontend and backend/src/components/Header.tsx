...
export default function Header({ currentPersona, setPersona, kaizens, openRedflagsCount, onMenuClick }: HeaderProps) {
  const pendingCount = kaizens.filter(k => k.status === 'pending' || k.status === 'submitted').length;
  const approvedCount = kaizens.filter(k => ['approved', 'good_point', 'closed'].includes(k.status)).length;
  const totalSavings = kaizens
    .filter(k => ['approved', 'good_point', 'closed'].includes(k.status))
    .reduce((sum, k) => sum + (Number(k.costSave) || 0), 0);
