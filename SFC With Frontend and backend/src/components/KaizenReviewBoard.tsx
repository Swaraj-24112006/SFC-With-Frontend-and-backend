...
export default function KaizenReviewBoard({ kaizens, onUpdateKaizen }: KaizenReviewBoardProps) {
  ...
  // Helper: normalize backend lowercase status to display form
  const normalizeStatus = (s: string): string => {
    const map: Record<string, string> = {
      draft: 'Pending', submitted: 'Pending', pending: 'Pending',
      approved: 'Approved', good_point: 'Good Point',
      rejected: 'Rejected', rework: 'Rework', closed: 'Closed',
    };
    return map[s?.toLowerCase()] ?? s;
  };

  // Helper: normalize classification
  const normalizeClassification = (c: string): string => {
    const map: Record<string, string> = {
      kaizen: 'Kaizen', good_point: 'Good Point', pending: 'Pending', none: 'None'
    };
  ...
  const filteredKaizens = kaizens.filter(k => {
    ...
    // Status
    const matchesStatus = filterStatus === 'All' || normalizeStatus(k.status) === filterStatus;
    ...
    // Classification
    const matchesClassification = filterClassification === 'All' || normalizeClassification(k.classification) === filterClassification;

    return matchesSearch && matchesStatus && matchesMonth && matchesMinifactory && matchesClassification;
  });


  // Committee member names (pre-login placeholder list)
  const COMMITTEE_MEMBERS = [
    'Rajesh Patil (Supervisor)',
    'Amit Mehta (Kaizen Lead)',
    'Vijay Deshmukh (Area Leader)',
    'Sunita Rao (Quality Lead)',
    'Vikram Sen (Safety Lead)',
    'Arjun Mehra (Process Lead)',
    'Sanjay Patil (Senior Machinist)',
  ];

  // Open Review Audit modal for a specific Kaizen
  const openReviewModal = (k: Kaizen) => {
    setReviewModalKaizen(k);
    // Normalize classification — backend returns 'good_point', frontend uses 'Good Point'
    const rawClass = k.classification || 'Pending';
    const classMap: Record<string, any> = {
      kaizen: 'Kaizen', good_point: 'Good Point', pending: 'Pending', none: 'None',
    };
    setClassification(classMap[rawClass.toLowerCase()] ?? rawClass as any);
    setStatus('Approved'); // Default decision is Approve
    setRemark(k.remark || '');
    setCostSave(k.costSave || 0);
    setApprovedBy(k.approvedBy || COMMITTEE_MEMBERS[0]);
    setVerifiedBy(k.verifiedByName || COMMITTEE_MEMBERS[1]);
  ...
  const handleSaveReviewModal = () => {
    ...
    // If user left status as 'Pending' but chose a classification, infer final status
    if ((status as string) === 'Pending' && (classification === 'Kaizen' || classification === 'Good Point')) {
    ...
    setSuccessMessage(`✅ Decision saved for ${reviewModalKaizen.srNo}! Updating database...`);
    setTimeout(() => {
      setSuccessMessage(null);
  ...
  // Status-aware pending / approved lists — handle both Django lowercase and Title Case
  const PENDING_STATUSES = ['pending', 'submitted', 'draft', 'Pending'];
  const APPROVED_STATUSES = ['approved', 'good_point', 'Approved', 'Good Point'];
  const pendingList = kaizens.filter(k => PENDING_STATUSES.includes(k.status));
  const approvedList = kaizens.filter(k => APPROVED_STATUSES.includes(k.status));
                    ...
                    >
                      ...
                      <td className="p-2 border-r border-slate-200 text-center bg-white group-hover:bg-amber-50/40 sticky left-0 z-10 shadow-xs">
                        <div className="flex items-center justify-center space-x-1">
                          ...
                          {APPROVED_STATUSES.includes(k.status) || k.status === 'rejected' ? (
                            <button
                              type="button"
                              disabled
                      ...
                      <td className="p-3 border-r border-slate-200 text-center">
                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-extrabold font-mono uppercase ${
                          APPROVED_STATUSES.includes(k.status)
                            ? k.status === 'good_point' || k.status === 'Good Point'
            ...
            <div className="p-4 bg-slate-50 text-xs grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              <div className="md:col-span-4 border-l border-slate-200 pl-4 space-y-2 font-mono text-[11px]">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-slate-900">{selectedKaizen.verifiedByName || 'Pending'}</span>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-slate-900 text-white border border-slate-800 rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs font-sans">
              ...
              {/* Sign-offs — dropdown list of committee members */}
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
