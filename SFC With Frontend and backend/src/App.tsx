...
export default function App() {
  ...
  // Normalize a raw API Kaizen object to match the frontend Kaizen interface.
  const normalizeKaizen = (raw: any): any => {
    const b = raw.benefits || {};
    return {
      ...raw,
      id: String(raw.id),
      title: raw.title || '',
      problemBefore: raw.problemBefore ?? raw.problem_before ?? '',
      counterMeasureAfter: raw.counterMeasureAfter ?? raw.counter_measure_after ?? '',
      area: raw.area || '',
      miniFactory: raw.miniFactory ?? raw.mini_factory ?? raw.minifactory ?? '',
      location: raw.location || '',
      machine: raw.machine || '',
      closingTargetDate: raw.closingTargetDate ?? raw.closing_target_date ?? '',
      implementationDate: raw.implementationDate ?? raw.implementation_date ?? raw.implementedDate ?? '',
      costSave: typeof raw.costSave === 'string' ? parseFloat(raw.costSave) || 0 : (raw.costSave ?? 0),
      ideaBy: raw.ideaBy ?? raw.idea_by ?? '',
      implementedBy: raw.implementedBy ?? raw.implemented_by ?? '',
      preparedBy: raw.preparedBy ?? raw.prepared_by ?? '',
      approvedBy: raw.approvedBy ?? raw.approved_by ?? '',
      verifiedByName: raw.verifiedByName ?? raw.verified_by_name ?? raw.verifiedBy ?? '',
      status: raw.status || 'draft',
      classification: raw.classification || 'pending',
      remark: raw.remark ?? raw.remarks ?? '',
      result: raw.result ?? '',
      photoBefore: raw.photoBefore ?? raw.photo_before ?? '',
      photoAfter: raw.photoAfter ?? raw.photo_after ?? '',
      impactAssessment: raw.impactAssessment ?? raw.impactAssessmentData ?? raw.impact_assessment_data ?? undefined,
      benefits: {
        p: b.productivity ?? b.p ?? false,
        q: b.quality     ?? b.q ?? false,
        c: b.cost        ?? b.c ?? false,
        d: b.delivery    ?? b.d ?? false,
        s: b.safety      ?? b.s ?? false,
        m: b.morale      ?? b.m ?? false,
      },
    };
  };
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Fetch Kaizens from Django Backend (paginated response: {count, results})
      const resK = await fetch('/api/v1/kaizens/?page_size=200');
      const dataK = await resK.json();
      // DRF pagination returns {count, results}; our custom endpoints return {success, data}
      const rawList = dataK.results ?? dataK.data ?? [];
      const kaizenList = rawList.map(normalizeKaizen);
      if (kaizenList.length >= 0) setKaizens(kaizenList);
  ...
  const handleAddKaizen = async (newKaizen: Partial<Kaizen>) => {
    try {
      ...
      const djangoPayload: Record<string, any> = {
        ...
        area: newKaizen.area,
        mini_factory: newKaizen.miniFactory || (newKaizen as any).minifactory || 'MF1',
        location: newKaizen.location,
        machine: newKaizen.machine,
        closing_target_date: newKaizen.closingTargetDate || null,
        implementation_date: newKaizen.implementationDate || (newKaizen as any).implementedDate || null,
      ...
      if (data.success) {
        setKaizens(prev => [normalizeKaizen(data.data), ...prev]);
  ...
  // Update Kaizen — routes committee decisions to the correct workflow endpoint
  const handleUpdateKaizen = async (id: string, updatedFields: Partial<Kaizen>) => {
    try {
      const rawStatus = (updatedFields.status ?? '').toLowerCase();
      const isWorkflowAction = ['approved', 'good point', 'good_point', 'rejected', 'rework', 'submitted'].includes(rawStatus);

      // ── Step 1: Always save committee fields (benefits, costSave, verifiedByName, etc.)
      //    via committee-patch BEFORE the workflow status change so nothing is lost
      if (isWorkflowAction) {
        const patchBody: Record<string, any> = {};
        if (updatedFields.benefits !== undefined) patchBody.benefits = updatedFields.benefits;
        if (updatedFields.costSave !== undefined) patchBody.cost_save = updatedFields.costSave;
        if (updatedFields.remark !== undefined) patchBody.remark = updatedFields.remark;
        if (updatedFields.approvedBy !== undefined) patchBody.approved_by = updatedFields.approvedBy;
        if (updatedFields.verifiedByName !== undefined) patchBody.verified_by_name = updatedFields.verifiedByName;

        if (Object.keys(patchBody).length > 0) {
          await fetch(`/api/v1/kaizens/${id}/committee-patch/`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
      ...
      // ── Step 2: Route to the correct workflow / field-patch endpoint
      let endpoint = `/api/v1/kaizens/${id}/committee-patch/`;
      let method = 'PATCH';
      let body: any = {};

      if (rawStatus === 'approved' || rawStatus === 'good point' || rawStatus === 'good_point') {
        endpoint = `/api/v1/kaizens/${id}/approve/`;
        method = 'POST';
        body = {
          classification: (rawStatus === 'good point' || rawStatus === 'good_point') ? 'good_point' : 'kaizen',
          remarks: updatedFields.remark || '',
          approved_by: updatedFields.approvedBy || '',
          verified_by: updatedFields.verifiedByName || '',
        };
      } else if (rawStatus === 'rejected') {
        endpoint = `/api/v1/kaizens/${id}/reject/`;
        method = 'POST';
        body = {
          reason: updatedFields.remark || 'Rejected by committee',
          approved_by: updatedFields.approvedBy || '',
          verified_by: updatedFields.verifiedByName || '',
        };
      } else if (rawStatus === 'rework') {
        endpoint = `/api/v1/kaizens/${id}/rework/`;
        method = 'POST';
        body = {
          remarks: updatedFields.remark || 'Please revise and resubmit.', // ReworkSerializer expects 'remarks'
        };
        ...
        // Pure committee-patch: impactAssessment, costSave, benefits, remark, etc.
        body = {};
        if (updatedFields.impactAssessment !== undefined) body.impact_assessment_data = updatedFields.impactAssessment;
        if (updatedFields.costSave !== undefined) body.cost_save = updatedFields.costSave;
        if (updatedFields.remark !== undefined) body.remark = updatedFields.remark;
        if (updatedFields.result !== undefined) body.result = updatedFields.result;
        if (updatedFields.benefits !== undefined) body.benefits = updatedFields.benefits;
        if (updatedFields.approvedBy !== undefined) body.approved_by = updatedFields.approvedBy;
        if (updatedFields.verifiedByName !== undefined) body.verified_by_name = updatedFields.verifiedByName;
      }
      ...
      if (data.success) {
        fetchAllData(); // Re-fetch & re-normalize to sync DB → UI
        if (inspectKaizen?.id === id) setInspectKaizen({ ...inspectKaizen, ...updatedFields });
      } else {
        console.error('Kaizen update failed:', data);
        alert(`Update failed: ${data?.error?.message || JSON.stringify(data)}`);
      }
    } catch (err) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 border-b border-slate-300 divide-y md:divide-y-0 md:divide-x divide-slate-300">
                  ...
                  <div className="md:col-span-3 p-4 bg-slate-50 font-mono text-[10px] space-y-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-slate-900 font-bold text-[11px]">{inspectKaizen.miniFactory || (inspectKaizen as any).minifactory}</span>
                    </div>
                    <div>
