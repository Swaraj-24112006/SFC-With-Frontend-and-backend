...
export interface Kaizen {
  ...
  area: string;
  miniFactory: string;    // API returns miniFactory (not minifactory)
  location: string;
  machine: string;
  closingTargetDate: string;
  implementationDate: string;  // API returns implementationDate (not implementedDate)
  costSave: number;
  benefits: {
    p: boolean; // Productivity  (mapped from API: productivity)
    q: boolean; // Quality       (mapped from API: quality)
    c: boolean; // Cost          (mapped from API: cost)
    d: boolean; // Delivery      (mapped from API: delivery)
    s: boolean; // Safety        (mapped from API: safety)
    m: boolean; // Morale        (mapped from API: morale)
  };
  ideaBy: string;
  implementedBy: string;
  preparedBy: string;
  approvedBy: string;
  verifiedByName: string;   // API returns verifiedByName (not verifiedBy)
  status: string;           // Backend values: draft, submitted, pending, approved, good_point, rejected, rework, closed
  classification: string;   // Backend values: kaizen, good_point, pending, none
  remark: string;
  photoBefore: string;
  photoAfter: string;
  result: string;
