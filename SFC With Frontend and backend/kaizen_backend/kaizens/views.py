...
class KaizenViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Only drafts and rework can be edited
        if not instance.is_editable:
            raise KaizenAPIException(
                message=f'Kaizen in "{instance.get_status_display()}" status cannot be edited.',
                code='NOT_EDITABLE',
                status_code=403,
            )
        ...
        if instance.created_by != request.user and not (
            ...
            raise KaizenAPIException(
                message='You can only edit your own Kaizens.',
    ...
    @action(detail=True, methods=['patch'], url_path='committee-patch')
    def committee_patch(self, request, pk=None):
        """
        PATCH /api/v1/kaizens/<id>/committee-patch/
        Committee-only update: saves impact_assessment, cost_save, remark,
        benefits, and other fields that should be updatable regardless of
        the kaizen's is_editable status.
        """
        kaizen = self.get_object()
        allowed_fields = {
            'impact_assessment_data', 'cost_save', 'remark', 'result',
            'approved_by', 'verified_by_name', 'closing_target_date',
            'implementation_date',
        }

        # Apply direct scalar fields
        for field in allowed_fields:
            if field in request.data:
                setattr(kaizen, field, request.data[field])

        # Handle benefits p/q/c/d/s/m → productivity/quality/cost/delivery/safety/morale
        benefits_payload = request.data.get('benefits')
        if benefits_payload and isinstance(benefits_payload, dict):
            from .models import KaizenBenefit
            benefit, _ = KaizenBenefit.objects.get_or_create(kaizen=kaizen)
            mapping = {
                'p': 'productivity', 'q': 'quality', 'c': 'cost',
                'd': 'delivery', 's': 'safety', 'm': 'morale',
            }
            for short, long in mapping.items():
                if short in benefits_payload:
                    setattr(benefit, long, benefits_payload[short])
            benefit.save()
        ...
        return Response({
            'success': True,
            'message': 'Kaizen updated by committee.',
            'data': serializer.data,
        })
