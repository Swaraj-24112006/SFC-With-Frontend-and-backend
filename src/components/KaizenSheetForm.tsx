...
export default function KaizenSheetForm({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submissionPayload: Partial<Kaizen> & { photoBeforeFile?: File; photoAfterFile?: File } = {
      ...
      machine,
      suggestionDate,
      closingTargetDate: closingTargetDate || new Date(Date.now() + 7 * 24 * 3600 * 1000).toISOString().split('T')[0],
    ...
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden max-w-5xl mx-auto my-6">
      ...
      <CameraModal
        ...
        onCapture={(img) => {
          ...
          // Convert base64 to File object for backend upload
          try {
            ...
            const file = new File([blob], `camera_${cameraTarget}_${Date.now()}.${ext}`, { type: mime });
            if (cameraTarget === 'before') setPhotoBeforeFile(file);
            else if (cameraTarget === 'after') setPhotoAfterFile(file);
          } catch (e) {
            console.warn('Could not convert camera image to File:', e);
          }
        }}
