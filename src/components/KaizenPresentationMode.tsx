              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-slate-300 rounded-2xl p-6 shadow-sm space-y-3 flex flex-col justify-between">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                    ...
                    {kaizen.photoBeforeUrl && (
                      <button
                        type="button"
                        onClick={() => setZoomedPhoto({ url: kaizen.photoBeforeUrl!, title: `BEFORE: ${kaizen.title}` })}
                  ...
                  <div 
                    onClick={() => kaizen.photoBeforeUrl && setZoomedPhoto({ url: kaizen.photoBeforeUrl, title: `BEFORE IMPROVEMENT: ${kaizen.title}` })}
                  ...
                  >
                    {kaizen.photoBeforeUrl ? (
                      <>
                        <img src={kaizen.photoBeforeUrl} alt="Photo Before" className="max-h-full max-w-full object-contain rounded-lg group-hover:scale-102 transition-transform" referrerPolicy="no-referrer" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-slate-300 rounded-2xl p-6 shadow-sm space-y-3 flex flex-col justify-between">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                    ...
                    {kaizen.photoAfterUrl && (
                      <button
                        type="button"
                        onClick={() => setZoomedPhoto({ url: kaizen.photoAfterUrl!, title: `AFTER: ${kaizen.title}` })}
                        className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-mono font-bold transition flex items-center space-x-1 cursor-pointer"
                      >
                        <ZoomIn className="w-4 h-4" />
                        <span>Enlarge</span>
                  ...
                  <div 
                    onClick={() => kaizen.photoAfterUrl && setZoomedPhoto({ url: kaizen.photoAfterUrl, title: `AFTER IMPROVEMENT: ${kaizen.title}` })}
                  ...
                  >
                    {kaizen.photoAfterUrl ? (
                      <>
                        <img src={kaizen.photoAfterUrl} alt="Photo After" className="max-h-full max-w-full object-contain rounded-lg group-hover:scale-102 transition-transform" referrerPolicy="no-referrer" />
                        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white font-mono text-xs font-bold space-x-1">
                          <ZoomIn className="w-4 h-4" />
                          <span>Click to Zoom High-Res</span>
                        </div>
